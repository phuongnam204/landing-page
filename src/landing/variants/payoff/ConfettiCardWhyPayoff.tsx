...
function ProgramsMiniCarousel({ programs, highlightId }: { programs: Program[]; highlightId?: string }) {
  ...
  return (
    <div className="flex flex-col" style={{ height: '320px' }}>
      ...
      >
        {activeIndex > 0 && (
          <button onClick={prev} className="absolute left-1 top-1/2 -translate-y-1/2 z-20 w-7 h-7 rounded-full bg-[var(--lp-bg-card)] shadow-md flex items-center justify-center text-cta/60 text-xs">&lsaquo;</button>
        )}
        {activeIndex < sorted.length - 1 && (
          <button onClick={next} className="absolute right-1 top-1/2 -translate-y-1/2 z-20 w-7 h-7 rounded-full bg-[var(--lp-bg-card)] shadow-md flex items-center justify-center text-cta/60 text-xs">&rsaquo;</button>
        )}
        {sorted.map((program, idx) => (
          <div key={program.id} className="absolute top-2 bottom-2 left-1/2" style={{ width: '76%', ...getCardStyle(idx) }}>
            <ProgramCard
              program={program}
              selected={false}
              isSuggested={program.id === highlightId}
              onSelect={() => {}}
            />
          </div>
        ))}
...
function WhySection({ conditionId, onScrollDown }: { conditionId: ConditionId; onScrollDown: () => void }) {
  ...
  return (
    <div className="max-w-lg md:max-w-3xl mx-auto px-5 py-10 flex flex-col gap-6">
      ...
      <blockquote className="bg-[var(--lp-accent)]/5 border-l-4 border-[var(--lp-accent)] px-4 py-3.5 rounded-r-lg">
    ...
    <div className="h-[100dvh] w-full bg-[var(--lp-bg-payoff)] overflow-y-auto">
      ...
      <div className="relative min-h-[100dvh] flex items-center justify-center px-5 overflow-hidden">
        ...
        >
          ...
          <p className="text-sm md:text-base text-cta/70 leading-snug px-3 py-2.5 bg-[var(--lp-accent)]/5 border-l-2 border-[var(--lp-accent)] rounded-r-lg mb-5">
      ...
      {/* Section 4: Feature + program mini-carousel + final CTA */}
      <div ref={featureRef}>
