...
export function PlayfulImmersivePrograms({ suggestedPrograms, onContinue }: ProgramsSlotProps) {
  return (
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex flex-col gap-4">
          {suggestedPrograms.map(sp => (
            <div
              key={sp.program.id}
              className="rounded-lg border border-[var(--lp-border)] p-5 flex flex-col gap-3"
              style={{ background: 'var(--lp-pastel-pink)' }}
            >
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className="font-bold text-base md:text-lg text-cta">{sp.program.name}</h3>
                {sp.score >= 2 && (
                  <span className="text-xs font-semibold bg-white/60 text-[var(--lp-accent)] rounded-full px-2.5 py-0.5">
                    Gợi ý
                  </span>
                )}
              </div>
