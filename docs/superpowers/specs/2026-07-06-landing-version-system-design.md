...
Theme chỉ là một CSS class đặt ở root của `LandingFlow`, nơi định nghĩa một bộ CSS variables.
Các variant không biết theme là gì — chúng chỉ đọc variables. Chuyển màu cứng sang variable
khi migrate: thay `bg-[#FFEFF4]` thành `bg-[var(--lp-bg-hero)]`.

Recipe khai báo theme bằng một field, `theme` là tùy chọn — nếu không khai báo thì dùng
`blossom` làm mặc định:
...
### Phạm vi kiểm soát của theme — quan trọng

Sau khi quét toàn bộ codebase, màu cứng trong repo chia thành **ba nhóm với bản chất khác nhau**,
và theme variables chỉ nên kiểm soát một nhóm trong đó:

**Nhóm 1 — Theme kiểm soát (tokenize):** Màu nền từng screen, màu primary/accent/border cho
UI chrome (button, card, input, text chính), blob decorative trong `gameShell.tsx`, gradient
nền. Đây là màu "thuộc về cảm giác thị giác" của landing — thay theme thì những màu này thay.

**Nhóm 2 — Theme KHÔNG kiểm soát — màu dữ liệu:** Màu của từng condition/program được tính
từ `cond.color` trong `src/content/catalog.ts` — badge màu theo loại da, dải header card theo
`tintColor`. Đây là màu nhận diện của từng loại bệnh da, thuộc về nội dung, không phải giao
diện. Đổi theme không được đổi màu này.

**Nhóm 3 — Theme KHÔNG kiểm soát — màu scene minigame:** `#7A9EBB` (bầu trời), `#E8A57E`
(đất/da), pattern địa hình trong `GameScene.tsx`. Đây là màu narrative của thế giới trong
game — chúng phải nhất quán dù theme nào, vì chúng mô tả scene chứ không phải UI.

### Bộ CSS variables chuẩn

```css
/* src/landing/themes.css */

.theme-blossom {               /* visual hiện tại — baseline */
  --lp-bg-hero:      #FFEFF4;
  --lp-bg-minigame:  #EDE9FF;
  --lp-bg-payoff:    #A8E6CF;
  --lp-bg-programs:  #C7CEEA;
  --lp-bg-card:      #ffffff;
  --lp-primary:      #2D2640;  /* cta navy */
  --lp-accent:       #7C3AED;
  --lp-border:       #B6BCEE;
  --lp-blob-1:       #FFB8D4;
  --lp-blob-2:       #B39DFF;
  --lp-blob-3:       #8FE3BC;
  --lp-radius-card:  20px;     /* rounded-soft hiện tại */
  --lp-radius-btn:   20px;
}
...
.theme-midnight {              /* dark-first chủ động */
  --lp-bg-hero:      #0f0c1a;
...
### Rủi ro kỹ thuật cần xử lý trong bước migrate

`gameShell.tsx` hiện dùng `darkMode: 'media'` + class `dark:hidden`/`hidden dark:block` để
swap hai bộ blob màu (pastel light ↔ tím tối). Khi `theme-midnight` được áp dụng qua CSS
class, hai cơ chế này sẽ **xung đột**: OS không nhất thiết ở dark mode nhưng recipe chọn
midnight. Giải pháp: trong bước migrate, xóa logic `dark:hidden`/`hidden dark:block` khỏi
gameShell, để blob màu đọc từ `--lp-blob-1/2/3` — theme nào thì blob màu đó.

### Những gì cần làm trong đợt migrate

- Định nghĩa `src/landing/themes.css` với năm theme trên.
- Khi di chuyển từng screen ra variant (bước 3 trong migrate), đồng thời thay màu cứng nhóm 1
  bằng variables — tay đang đụng vào file đó rồi, chi phí tăng thêm tối thiểu.
- Xử lý xung đột `dark:hidden` trong `gameShell.tsx` (xem rủi ro trên).
- Giữ nguyên màu nhóm 2 (dữ liệu) và nhóm 3 (scene) — không tokenize.
